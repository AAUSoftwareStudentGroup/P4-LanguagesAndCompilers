...
namespace Compiler.Tests
{
    public class LexerTests
    {
        ...
        {
            // Initialise Lexer
            Lexer l = new Lexer(File.ReadAllText(AppContext.BaseDirectory + "/tang.tokens.json"));

            // Read from test file
            IEnumerable<Token> tokens = l.Analyse(@"int16 a = 1
bool on = true
...
while(on == true)
    a = a + 1");
        ...
        {
            // Initialise Lexer
            Lexer le = new Lexer(File.ReadAllText(AppContext.BaseDirectory + "/tang.tokens.json"));

            // Read from another file, tokens should be SimpleType Identifier Assign Number eof (int16 a = 1)
            IEnumerable<Token> tokens = le.Analyse(@"int16 a = 1
bool on = true
